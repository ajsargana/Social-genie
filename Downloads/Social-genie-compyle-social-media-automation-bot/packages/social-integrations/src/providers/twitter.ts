...
export class TwitterIntegration extends SocialPlatformIntegration {
  ...
  async postContent(post: ScheduledPost): Promise<PostResult> {
    try {
      ...
      const payload: TwitterTweetRequest = {
        text: tweetText
      };
      ...
      const response = await axios.post<TwitterTweetResponse>(
        `${this.baseUrl}/tweets`,
        payload,
        {
          headers: {
  ...
  async getAnalytics(postId: string): Promise<PostAnalytics> {
    try {
      ...
      const response = await axios.get<TwitterMetricsResponse>(
  ...
  private adaptContentForTwitter(content: string, capabilities: any): string {
  ...
  private async uploadMedia(mediaUrl: string, token: string): Promise<string> {
    try {
